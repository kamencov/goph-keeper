...
var (
	...
	errSaveText  = errors.New("failed to save cards data in database")
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedGetUserIDErr != nil {
				...
				if tt.expectedSaveCardsErr != nil {
					db.EXPECT().SaveCardsInDatabase(ctx, 1, gomock.Any()).Return(tt.expectedSaveCardsErr).AnyTimes()
				} else {
					db.EXPECT().SaveCardsInDatabase(ctx, 1, gomock.Any()).Return(nil).AnyTimes()

					if tt.expectedGetIDTaskErr != nil {
						db.EXPECT().GetIDTaskCards(ctx, gomock.Any(), 1, gomock.Any()).Return(0, tt.expectedGetIDTaskErr).AnyTimes()

					} else {
						db.EXPECT().GetIDTaskCards(ctx, gomock.Any(), 1, gomock.Any()).Return(1, nil).AnyTimes()
			...
			err := testService.SaveCards(ctx, tt.token, tt.data)
